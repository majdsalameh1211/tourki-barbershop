...
const NoShowTable = () => {
  ...
  const toggleSuspend = (id) => {
    setClients(prev => prev.map(client => {
      if (client.id === id) {
        // Toggle logic
        const newStatus = !client.isSuspended;
        // Optional: Add alert or confirmation logic here
        return { ...client, isSuspended: newStatus };
      }
      return client;
    }));
  };
  ...
  return (
    <div className="noshow-card">
      ...
      {/* List */}
      {clients.map(client => (
        <div key={client.id} className={`ns-row ${client.isSuspended ? 'suspended' : ''}`}>
