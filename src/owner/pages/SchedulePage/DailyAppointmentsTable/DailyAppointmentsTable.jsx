...
const DailyAppointmentsTable = ({ date, appointments, onSelect }) => {
  ...
  return (
    <div className="daily-table-container">
      ...
      <div className="table-body">
        {data.map((appt) => (
          <div key={appt.id} className="appt-row" onClick={() => onSelect(appt)}>
            
            <div className="col-time">{appt.time}</div>
            
            <div className="col-name">{appt.name}</div>
            
            <a href={`tel:${appt.phone}`} className="col-phone" onClick={(e) => e.stopPropagation()}>
              {appt.phone}
            </a>
            
            <div className="col-whatsapp">
              <svg 
                className={`wa-icon ${appt.whatsapp ? 'active' : ''}`} 
                viewBox="0 0 24 24" 
                fill="currentColor"
              >
            ...
            <div className="col-action">
              <button className="view-btn" title="צפה">
